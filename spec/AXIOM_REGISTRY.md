...
- **Enforced by**: `kernel._machine_resource_authorized` (`kernel.py:118–131`), invoked at `kernel.py:78`. A delegated resource is legitimately usable only if the machine's *owner* owns it (`graph.human_owns_resource(owner, resource)`, the A5 containment) **or** the resource's true owner is an affected party who gave valid consent. A5 is thus folded into the A7 check rather than carried as a standalone field — there is no explicit `MachineScope` set in `model.py`.
- **Status**: PARTIAL (the *containment* is enforced via the A7 path; there is no first-class scope object, so A5 cannot be checked independently of a concrete resource use).
