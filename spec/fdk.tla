...
(*                                                                         *)
(* STATUS — these invariants are now MODEL-CHECKED. The finite instance     *)
(* spec/MC_fdk.tla (+ MC_fdk.cfg) instantiates the CONSTANTS and runs this   *)
(* module through TLC: TypeOK, Safety, NoLegitimateSlavery,                 *)
(* NoMachineSovereignty, and NoLaunderingViaResistance all hold —           *)
(* "Model checking completed. No error has been found." (TLC2 2.19).        *)
(* What remains open: TLAPS discharge of the THEOREMs below (an unbounded    *)
(* proof, not a finite check), and the model<->Python refinement (covered    *)
(* by tests/test_lean_refinement.py for the categorical core). The Lean      *)
(* proofs (formal/Fdk.lean) and the executable Hypothesis suite              *)
(* (tests/test_theorems.py) are the companion machine-checked evidence.     *)
(*                                                                         *)
